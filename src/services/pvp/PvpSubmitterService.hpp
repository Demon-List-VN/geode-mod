...
class PvpSubmitterService {
    ...
    struct State {
        ...
        float best = 0;
        float pendingBest = 0;
        float pauseProgress = 0;
        std::chrono::steady_clock::time_point deathCountLastSubmitAt;
        ...
        explicit State(int levelID = 0)
            : levelID(levelID), deathCountLastSubmitAt(std::chrono::steady_clock::now()) {
        }
    };
    ...
    static void submitPauseState(std::shared_ptr<State> state, bool paused, float progress);
